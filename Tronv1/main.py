...
class NeuralNetwork:
    ...
    def __init__(self, num_actions, replay_memory, number):
        """
        :param num_actions:
        ...
        net = tf.layers.conv2d(inputs=net, name='layer_conv1' + str(number),
                               filters=16, kernel_size=3, strides=2,
                               padding=padding,
                               kernel_initializer=init, activation=activation)

        # Second convolutional layer.
        net = tf.layers.conv2d(inputs=net, name='layer_conv2'+ str(number),
                               filters=32, kernel_size=3, strides=2,
                               padding=padding,
                               kernel_initializer=init, activation=activation)

        # Third convolutional layer.
        net = tf.layers.conv2d(inputs=net, name='layer_conv3'+ str(number),
        ...
        net = tf.layers.dense(inputs=net, name='layer_fc1'+ str(number), units=1024,
                              kernel_initializer=init, activation=activation)

        # Second fully-connected layer.
        net = tf.layers.dense(inputs=net, name='layer_fc2'+ str(number), units=1024,
                              kernel_initializer=init, activation=activation)

        # Third fully-connected layer.
        net = tf.layers.dense(inputs=net, name='layer_fc3'+ str(number), units=1024,
                              kernel_initializer=init, activation=activation)

        # Fourth fully-connected layer.
        net = tf.layers.dense(inputs=net, name='layer_fc4'+ str(number), units=1024,
                              kernel_initializer=init, activation=activation)

        # Final fully-connected layer.
        net = tf.layers.dense(inputs=net, name='layer_fc_out'+ str(number), units=num_actions,
    ...
    def optimize(self, min_epochs=1.0, max_epochs=10,
                 batch_size=32, loss_limit=0.015,
                 learning_rate=1e-3):
        """
...
class Agent:
    ...
    def __init__(self, env_name, training, player_number, render=False, use_logging=True):
        """
...
def runNN():
    ...
    count_states = agent1.model.get_count_states()
    count_states2 = agent2.model.get_count_states()

    # Counter for the number of episodes we have processed.
    count_episodes = agent1.model.get_count_episodes()
    count_episodes2 = agent2.model.get_count_episodes()
    ...
    if num_episodes2 is None:
        num_episodes2 = float('inf')
    else:
        num_episodes2 += count_episodes2
    ...
    while count_episodes <= num_episodes:
        ...
        if czy_reset:
            ...
            motion_tracer2 = MotionTracer(img2)
            ...
            count_episodes = agent1.model.increase_count_episodes()
            count_episodes2 = agent2.model.increase_count_episodes()
        ...
        state2 = motion_tracer2.get_state()
        ...
        q_values = agent1.model.get_q_values(states=[state])[0]
        q_values2 = agent2.model.get_q_values(states=[state2])[0]

        # Determine the action that the agent must take in the game-environment.
        # The epsilon is just used for printing further below.
        action, epsilon = agent1.epsilon_greedy.get_action(q_values=q_values,
                                                           iteration=count_states,
                                                           training=agent1.training)

        action2, epsilon2 = agent2.epsilon_greedy.get_action(q_values=q_values2,
                                                             iteration=count_states2,
                                                             training=agent2.training)
        ...
        img, reward, end_episode = env.step(action)

        action2 = action2 + 10
        img2, reward2, end_episode2 = env.step(action2)
        bonusReward = 0
        bonusReward2 = 0
        action2 = action2 - 10
       #env.render()
        ...
        motion_tracer2.process(image=img2)
        #print("End 1: " + str(end_episode) + " End 2: " + str(end_episode2))
        if(end_episode):
            bonusReward2 = 50
        elif(end_episode2):
            bonusReward = 50
        # Add the reward for the step to the reward for the entire episode.
        reward_episode = reward_episode + reward + bonusReward
        reward2_episode = reward2_episode + reward2 + bonusReward2
        ...
        count_states = agent1.model.increase_count_states()
        count_states2 = agent2.model.increase_count_states()

        if not agent1.training and agent1.render:
        ...
        if agent1.training:
            ...
            agent1.replay_memory.add(state=state,
                                   q_values=q_values,
                                   action=action,
                                   reward=reward,
                                   end_life=0,
            ...
            use_fraction = agent1.replay_fraction.get_value(iteration=count_states)

            # When the replay-memory is sufficiently full.
            if agent1.replay_memory.is_full() \
                    or agent1.replay_memory.used_fraction() > use_fraction:

                # Update all Q-values in the replay-memory through a backwards-sweep.
                agent1.replay_memory.update_all_q_values()

                # Log statistics for the Q-values to file.
                if agent1.use_logging:
                    agent1.log_q_values.write(count_episodes=count_episodes,
                                            count_states=count_states,
                                            q_values=agent1.replay_memory.q_values)
                ...
                learning_rate = agent1.learning_rate_control.get_value(iteration=count_states)
                loss_limit = agent1.loss_limit_control.get_value(iteration=count_states)
                max_epochs = agent1.max_epochs_control.get_value(iteration=count_states)
                ...
                agent1.model.optimize(learning_rate=learning_rate,
                                      loss_limit=loss_limit,
                                      max_epochs=max_epochs)

                # Save a checkpoint of the Neural Network so we can reload it.
                agent1.model.save_checkpoint(count_states)
                ...
                agent1.replay_memory.reset()

        if agent2.training:
            # Add the state of the game-environment to the replay-memory.
            agent2.replay_memory.add(state=state2,
                                     q_values=q_values2,
                                     action=action2,
                                     reward=reward2,
                                     end_life=0,
                                     end_episode=end_episode2)

            # How much of the replay-memory should be used.
            use_fraction2 = agent2.replay_fraction.get_value(iteration=count_states2)

            # When the replay-memory is sufficiently full.
            if agent2.replay_memory.is_full() \
                    or agent2.replay_memory.used_fraction() > use_fraction2:

                # Update all Q-values in the replay-memory through a backwards-sweep.
                agent2.replay_memory.update_all_q_values()

                # Log statistics for the Q-values to file.
                if agent2.use_logging:
                    agent2.log_q_values.write(count_episodes=count_episodes2,
                                              count_states=count_states2,
                                              q_values=agent2.replay_memory.q_values)

                # Get the control parameters for optimization of the Neural Network.
                # These are changed linearly depending on the state-counter.
                learning_rate2 = agent2.learning_rate_control.get_value(iteration=count_states2)
                loss_limit2 = agent2.loss_limit_control.get_value(iteration=count_states2)
                max_epochs2 = agent2.max_epochs_control.get_value(iteration=count_states2)

                # Perform an optimization run on the Neural Network so as to
                # improve the estimates for the Q-values.
                # This will sample random batches from the replay-memory.
                agent2.model.optimize(learning_rate=learning_rate2,
                                      loss_limit=loss_limit2,
                                      max_epochs=max_epochs2)

                # Save a checkpoint of the Neural Network so we can reload it.
                agent2.model.save_checkpoint(count_states2)

                # Reset the replay-memory. This throws away all the data we have
                # just gathered, so we will have to fill the replay-memory again.
                agent2.replay_memory.reset()


        if end_episode or end_episode2:
            # Add the episode's reward to a list for calculating statistics.
            agent1.episode_rewards.append(reward_episode)
            agent2.episode_rewards.append(reward2_episode)
            czy_reset = True
            bonusReward = 0
            bonusReward2 = 0

        # Mean reward of the last 30 episodes.
        if len(agent1.episode_rewards) == 0:
            # The list of rewards is empty.
            reward_mean = 0.0
        else:
            reward_mean = np.mean(agent1.episode_rewards[-30:])

        if len(agent2.episode_rewards) == 0:
            # The list of rewards is empty.
            reward_mean2 = 0.0
        else:
            reward_mean2 = np.mean(agent2.episode_rewards[-30:])


        if end_episode or end_episode2:
            # Log reward to file.
            if agent1.use_logging:
                agent1.log_reward.write(count_episodes=count_episodes,
                                        count_states=count_states,
            ...
            if agent2.use_logging:
                agent2.log_reward.write(count_episodes=count_episodes2,
                                        count_states=count_states2,
                                        reward_episode=reward2_episode,
                                        reward_mean=reward_mean2)

            # Print reward to screen.
            msg = "{0:4}:{1}\t Epsilon: {2:4.2f}\t Reward: {3:.1f}\t Episode Mean: {4:.1f}"
            print(msg.format(count_episodes2, count_states2, epsilon2,
                             reward2_episode, reward_mean2))

if __name__ == '__main__':
    # Description of this program.
    desc = "Reinformenct Learning (Q-learning) for Pablito Games using TensorFlow."
    ...
    parser.add_argument("--training", required=False, default=True,
    ...
    runNN()
    """
    agent1.run(num_episodes=num_episodes)
    agent2.run(num_episodes=num_episodes)
    """
    # Print statistics.
    rewards1 = agent1.episode_rewards
    rewards2 = agent2.episode_rewards
    print()  # Newline.
    ...
    print()  # Newline.
    print("Rewards for {0} episodes: ehe ".format(len(rewards1)))
    print("- Min:   ", np.min(rewards2))
    print("- Mean:  ", np.mean(rewards2))
    print("- Max:   ", np.max(rewards2))
    print("- Stdev: ", np.std(rewards2))
