...
if __name__ == '__main__':
    ...
    processes = []
    t = multiprocessing.Process(target=halo.start1(env_name,training,render,event,event2), args=())
    processes.append(t)
    t.start()
    x = multiprocessing.Process(target=halo.start2(env_name,training,render,event,event2), args=())
    processes.append(x)
    x.start()
    ...
    # Thread(target=halo.start1(env_name,training,render)).start()
    # Thread(target=halo.start2(env_name, training, render)).start()

    # th1 = threading.Thread(target=halo.start1(env_name,training,render)).start()
    # th2 = threading.Thread(target=halo.start2(env_name,training,render)).start()
    # print("ehhhh")
